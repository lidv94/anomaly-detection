...
def init_weights(layer, seed=42):
    """Deterministic per-layer initialization (like TensorFlow)."""
...
def train_autoencoder_checkpoint(model, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if current_lr != last_lr:
            print(f"[INFO] Current LR: {current_lr:.20f}")
            last_lr = current_lr
...
def load_pickled_autoencoder(checkpoint_path, model=None, device=None):
    ...
    if model is not None and "model_state_dict" in checkpoint:
        ...
        model.to(device)
        model.eval()  ### critical for deterministic predictions
...
def get_thresholds(recon_error, y_train_all, exp_name="default_all"):
    """
    Calculate fine-grained threshold cutoffs (e.g., 1.00–99.99 percentile)
    using only non-fraud (y=0) reconstruction errors.
    ...
    """
    # Create percentiles from 0.01 to 99.99 with 0.01 step
    percentiles = np.arange(0.01, 100, 0.01).round(2)
...
def run_evaluate_test_pipeline(trained_model, 
    ...
    # Step 4: Get embedding only test
    embeddings_test = extract_embeddings(trained_model, X_scaled_test)
    
    return recon_error_test, y_pred_test, result_test_df ,embeddings_test
